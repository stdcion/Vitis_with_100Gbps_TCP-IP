...
if have_state:
    add('     $display("  portState_a=%0d portState_b=%0d", portState_a, portState_b);')
if have_att:
    add('     $display("  listenAttempts_a=%0d listenAttempts_b=%0d",')
    add("              listenAttempts_a, listenAttempts_b);")
...
if have_state:
    add('     check("portState_a reflects work done inside the loop (!=0)",')
    add("           portState_a != 32'd0);")
if have_att:
    add('     check("listenAttempts_a matches the writes counted on the bus",')
    add("           listenAttempts_a == port_writes_a);")
if not (have_state and have_att):
    add('     $display("");')
