...
run_one () {
     ...
     # Файл тестбенча должен существовать. Без этой проверки xvlog получал бы
     # несуществующий путь, падал с невнятной ошибкой, и -- хуже -- при опечатке
     # в имени режима не запускалось бы НИЧЕГО, а скрипт печатал «всё зелёное».
     local last="${@: -1}"
     if [ ! -f "$last" ]; then
          echo ""
          echo "*** нет файла тестбенча: $last"
