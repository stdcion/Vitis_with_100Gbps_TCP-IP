...
// ═════════════════════════════════════════════════════════════════════════════
// ОТЛАДОЧНЫЙ VIO — ВРЕМЕННЫЙ, УДАЛИТЬ ПОСЛЕ ДИАГНОСТИКИ
// ═════════════════════════════════════════════════════════════════════════════
//
// ЗАЧЕМ. Три прошивки подряд дают одно и то же: enable=1 читается обратно, а
// listenAttempts=0 и portState=0. По сгенерированному RTL исключены все пять
// известных дефектов HLS (каналов 0, буферов 0, ap_start=1'b1, скаляр идёт
// проводом до сравнения), исключены неверный адрес, чужой битстрим и сброс.
// Стек при этом ЖИВОЙ: пинги идут, ARP отвечает.
//
// То есть цепочка «регистр -> провод -> ядро -> запрос в стек» рвётся в одном
// из четырёх мест, и по RTL их больше не различить: все стадии выглядят
// одинаково. Дальше гадать дороже, чем измерить.
//
// ЧТО РАЗЛИЧАЮТ ПРОБЫ (читать через vio_dump, он находит VIO сам):
//
//   enable_reg=1, enableA=0     -> рвётся ВНУТРИ ОБЁРТКИ (хотя симуляция
//                                  говорит обратное -- tb_probe_ctrl это
//                                  проверял на заглушке, а не на HLS-ядре)
//   enableA=1, lp_tvalid=0      -> ядро НЕ ПИШЕТ запрос: дефект в HLS,
//                                  скорее всего регион не перезапускается
//                                  (ap_sync_done -- И по всем 14 стадиям,
//                                  dual_echo_core.v:1337)
//   lp_tvalid=1, lp_tready=0    -> ядро пишет, а TOE НЕ ПРИНИМАЕТ: дефект в
//                                  стеке или в его настройке, не у нас
//   lp_tvalid=1, lp_tready=1    -> запрос ушёл, а portState всё равно 0 ->
//                                  смотреть port_status обратно
//
// Три исхода -- три разных лечения. Без этого следующая сборка снова была бы
// проверкой гипотезы, а не измерением.
//
// ПОЧЕМУ В ОБЁРТКЕ, А НЕ В BD. Здесь видны СРАЗУ обе стороны границы: и
// регистр из control_s_axi, и то, что реально приходит на порт HLS-ядра. Из
// BD второе недоступно -- там ядро уже чёрный ящик.
...
vio_dual_echo_dbg vio_dbg_inst (
     .clk        ( ap_clk                            ),
     .probe_in0  ( enable_reg                        ),  // 32: регистр 0x10
     .probe_in1  ( listenPortA_reg                   ),  // 32: регистр 0x18
     .probe_in2  ( portState_a                       ),  // 32: что выдаёт ядро
     .probe_in3  ( listenAttempts_a                  ),  // 32: сколько запросов
     .probe_in4  ( m_axis_tcp_listen_port_a_tvalid   ),  //  1: ядро пишет?
     .probe_in5  ( m_axis_tcp_listen_port_a_tready   ),  //  1: TOE принимает?
     .probe_in6  ( s_axis_tcp_port_status_a_tvalid   ),  //  1: стек ответил?
     .probe_in7  ( ap_rst_n                          )   //  1: контроль сброса
);
