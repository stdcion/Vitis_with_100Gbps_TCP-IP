...
//
// СЧИТАЕТСЯ В ПРОХОДАХ СТАДИИ, А НЕ В ТАКТАХ, и это различие уже дважды стоило
// ложных выводов. waitTimer инкрементируется один раз за проход, а проход при
// ap_ctrl_hs занимает НЕСКОЛЬКО тактов: измерено tb_core_ap_done -- ровно 3
// (999999 проходов региона на 3 000 000 тактов).
//
// При прежнем значении 1e6 повтор наступал через ~3 млн тактов, то есть ровно на
// границе прогона теста: writes оставался 1, и это выглядело как замерший регион,
// хотя ядро работало правильно. Отсюда правило: сверять порог с фактическим числом
// проходов, а не с частотой.
//
// 100 000 проходов x 3 такта / 165 МГц = ~1.8 мс. Стек отвечает за единицы
// микросекунд, так что запас трёхзначный; при этом повтор виден и в тесте, и на
// плате в разумное время.
...
// disable_start_propagation убран и здесь -- по той же причине, что на топе:
// он не даёт импульсу ap_start дойти до стадий, и барьер ap_sync_done остаётся
// взведённым. См. подробное пояснение у DATAFLOW в топ-функции.
#pragma HLS DATAFLOW
...
void hls_dual_echo_krnl(
               ...
               ap_uint<32>& portState_a,       // 0=не запрашивали 1=запрос 2=открыт
...
//
// НЕ УБИРАТЬ САМ DATAFLOW. Без него сборка проходит без единой ошибки и csynth
// молчит, а скаляры застревают в буфере state2.
//
// НО disable_start_propagation УБРАН, И ЭТО ИЗМЕРЕНО, А НЕ УГАДАНО.
//
// Pragma буквально означает «не раздавать ap_start стадиям». При ap_ctrl_none это
// было безвредно: ap_start региона всё равно зашит в единицу. При ap_ctrl_hs +
// auto_restart он ЛОМАЕТ ЗАПУСК -- импульс ap_start не доходит до стадий, значит
// регистры ap_sync_reg не сбрасываются и барьер ap_sync_done (И по ap_done всех
// стадий) держит регион после первого прохода.
//
// Измерено tb_core_ap_done на этой самой версии ядра:
//
//     writes a=1 b=1        <- один полезный проход вместо повторов
//     region ap_done cycles: 999999
//     verdict: every stage finishes, so ap_sync_done is evaluated every cycle
//
// Причём mode 0 (ap_start=1) и mode 1 (импульсы) дали ОДИНАКОВЫЙ результат --
// то есть auto_restart не помогал именно потому, что импульс до стадий не
// доходил.
//
// Скаляры от снятия pragma не страдают: они теперь s_axilite, то есть уже лежат
// в регистрах control_s_axi, а не приходят проводами извне. Защёлка state2, из-за
// которой pragma и появился, для s_axilite не проблема.
// ─────────────────────────────────────────────────────────────────────────────
#pragma HLS DATAFLOW
