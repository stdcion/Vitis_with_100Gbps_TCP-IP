...
int main()
{
     ...
     // ЗАПРОС ПОВТОРЯЕТСЯ, ПОКА СТЕК МОЛЧИТ. Это главное, что отличает нашу
     // версию от первой (та писала порт РОВНО ОДИН РАЗ) и от чего зависит,
     // откроется ли порт вообще: если единственный запрос попал в окно, когда
     // TOE ещё инициализирует таблицы, второй попытки не было бы никогда.
     //
     // Проверяем ФАКТ: стек не отвечает, а запросы продолжают приходить.
     int reqs_before = (int)m_axis_tcp_listen_port.size();
     run(20);
     int reqs_after = (int)m_axis_tcp_listen_port.size();
     check(reqs_after > reqs_before,
           "запрос ПОВТОРЯЕТСЯ, пока стек не подтвердил");
     // Опустошаем: иначе накопленные запросы помешают следующим сценариям.
     while (!m_axis_tcp_listen_port.empty()) m_axis_tcp_listen_port.read();
     ...
     // И ПОСЛЕ ОТКРЫТИЯ ЗАПРОСЫ ПРЕКРАЩАЮТСЯ. Иначе шина listen забивалась бы
     // без нужды каждый такт, а стек отвечал бы на каждый -- лишний трафик по
     // внутренним FIFO на всё время работы.
     while (!m_axis_tcp_listen_port.empty()) m_axis_tcp_listen_port.read();
     run(30);
     check(m_axis_tcp_listen_port.empty(),
           "после открытия порта запросы ПРЕКРАТИЛИСЬ");
