...
# ── СНАЧАЛА ДИАГНОСТИКА, ПОТОМ ОСТАНОВКА ────────────────────────────────────
#
# check_syntax печатает ошибки парсера С НОМЕРАМИ СТРОК. Стоять он должен ДО
# проверки top, а не после: в прогоне 25.08 я поставил его ниже, error до него
# не доходил, и лог опять не сказал, ЧТО именно не разобралось.
#
# Не фатален сам по себе: сообщение важнее остановки, а несобранный модуль
# всё равно поймает проверка top ниже.
puts ""
puts "=== check_syntax: ошибки парсера, если есть ==="
if {[catch {check_syntax -fileset sources_1} syn_err]} {
     puts "*** check_syntax: $syn_err"
} else {
     puts "  ошибок синтаксиса нет"
}
puts ""
...
#
# В этом ядре те 38 оказались безобидны (TSTRB + _ap_vld, см.
# ALLOW_UNCONNECTED ниже), но опираться на эту проверку предстоит при клонировании
# обёртки. Незамеченный неподключённый порт означает молча неработающую шину,
# которую на плате не отличить от «ядро не отвечает». Поэтому проверяются ОБА
# направления, а сознательно неподключаемые порты перечислены явно.
#
# ЧИСЛА УСТАРЕЛИ ПОСЛЕ ПРАВКИ 28.08. У ядра было 32 AXI-Stream порта; 23 из них
# убраны из сигнатуры вместе с шестью tie_off_*, потому что те писали в стек
# мусор при каждом перезапуске региона (3000 SYN к 0.0.0.0 на плате 27.08).
# Осталось 9 потоков -- те, что реально используют pp_listen и pp_echo. Убранные
# порты теперь заглушены КОНСТАНТАМИ в обёртке, как в iperf_krnl
# (src/hdl/user_krnl.sv:239). Обе проверки от этого не страдают: удалённых
# портов у IP больше нет, значит и сверять их не с чем.
puts ""
...
puts ""
puts "  0x80+      РЕГИСТРЫ ОБЁРТКИ, заданы localparam в"
puts "              src/hdl/hls_pp_dual_krnl_wrapper.sv:"
puts "                0x80 fifoRead(R)   0x84 fifoPop(W)"
puts "                0x88 fifoCount(R)  0x8c fifoOverflow(R)"
puts "                0x90 minWords(RW)  0x94 nfCountRx(R)"
puts "                0x98 nfDropRx(R)   0x9c nfCountTx(R)"
puts "                0xa0 nfDropTx(R)   0xa4 measDropped(R)"
puts "                0xa8 fifoClear(W)"
puts ""
