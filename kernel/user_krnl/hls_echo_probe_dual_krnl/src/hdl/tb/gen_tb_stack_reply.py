...
#
# ПРАВИЛО ПРАВКИ БЛОКОВ: внутри begin...end ВСЕ объявления (int/logic/reg) должны
# стоять ДО первого исполняемого оператора. Объявление после присваивания -- это
# СИНТАКСИЧЕСКАЯ ОШИБКА, xvlog даёт "VRFC 10-4982 syntax error near 'unsigned'",
# и по тексту ошибки не видно, что дело в порядке. Так сломался прогон 19.08:
# правка вставила снимок счётчика между двумя int unsigned.
#
# Локального xvlog нет, автоматически это не проверить (две попытки написать
# текстовую проверку дали десятки ложных срабатываний на рабочих файлах -- в
# области модуля объявления после always законны). Поэтому: правя блок, глазами
# проверьте, что объявления сгруппированы сверху.
...
add("int unsigned w_phase1 = 0;        // темп фазы 1 -- ориентир для остальных")
add("")
...
add("    w_phase1 = open_writes - w0;")
add("    if (w_phase1 > 0)")
add("        $display(\"    baseline retry rate: %0d cycles per attempt\", 1000000 / w_phase1);")
...
add("        // ВСЕ ОБЪЯВЛЕНИЯ -- ДО ПЕРВОГО ОПЕРАТОРА. В SystemVerilog объявление")
add("        // после исполняемой строки внутри блока -- синтаксическая ошибка")
add("        // (VRFC 10-4982 \"syntax error near unsigned\"), а не предупреждение.")
add("        // Именно так сломался прогон 19.08: присваивание w_settle0 попало")
add("        // между двумя int unsigned.")
add("        int unsigned w_before, d_before, w_after, w_settle0;")
add("        int unsigned settle = (RETRY_DELAY > 0) ? RETRY_DELAY * 4 : 400000;")
add("        w_settle0 = open_writes;   // снимок ДО ожидания")
add("        // Ждём, пока автомат ГАРАНТИРОВАННО уйдёт из WAIT_STATUS.")
add("        repeat (settle) @(posedge ap_clk);")
add("        // ПЕЧАТАЕМ ПРИРОСТ, А НЕ НАКОПЛЕННОЕ. Накопленное включает фазы 1-3")
add("        // и читается как результат этой фазы: в прогоне 19.08 было")
add("        // \"open_writes=10\", из которых 5 пришли из предыдущих фаз.")
add("        $display(\"    during %0d cycles of waiting: open_writes += %0d (total %0d)\",")
add("                 settle, open_writes - w_settle0, open_writes);")
add("")
add("        // ТЕМП ПОПЫТОК -- сверка с фазой 1 при ТЕХ ЖЕ условиях (стек молчит).")
add("        // Если он расходится в разы, дело не в позднем ответе, а в том, что")
add("        // темп повторов вообще не тот, что задаёт retryDelay.")
add("        if ((open_writes - w_settle0) > 0)")
add("            $display(\"    retry rate: %0d cycles per attempt (phase 1 gave %0d)\",")
add("                     settle / (open_writes - w_settle0),")
add("                     (w_phase1 > 0) ? 1000000 / w_phase1 : 0);")
