...
if have_state:
    ...
    add("    $display(\"    portState srazu posle sbrosa = %0d (dolzhno byt 0 ili 1)\", portState);")
    add("    check(\"do otveta steka port NE otkryt\", portState != 2,")
    add("          $sformatf(\"portState=%0d do reply_port_status\", portState));")
...
add("            $display(\"    (!) server ne poprosil port za 2M taktov -- POSLE etogo\");")
add("        else")
add("            $display(\"    server poprosil port (listen_writes %0d -> %0d)\",")
add("                     w1, listen_writes);")
add("        check(\"server zaprosil listen-port\", listen_writes != w1,")
