...
//
// 32 бита при 170 МГц переполняются за 25 секунд; разности берутся по модулю
...
#pragma HLS RESET variable=attempts
     ...
     // listen-порт. 100000 тактов ≈ 0.6 мс на 170 МГц.
...
void hls_echo_probe_dual_krnl(
               ...
               // счётчика (раз в 25 с на 170 МГц) измерение не портит.
