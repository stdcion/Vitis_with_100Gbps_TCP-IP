...
#pragma HLS RESET variable=attempts
     ...
     // Пауза между попытками: даём серверной половине время открыть listen-порт.
     //
     // СЧИТАЕТСЯ В ПРОХОДАХ СТАДИИ, А НЕ В ТАКТАХ. retryTimer инкрементируется
     // один раз за проход, а проход epd_core занимает ~3.5 такта (измерено
     // tb_core_ap_done: 857142 прохода на 3 млн тактов). Значит 100000 проходов
     // это ~350 000 тактов ≈ 2.1 мс на 165 МГц -- а не 0.6 мс, как считал прежний
     // комментарий.
     //
     // На этой арифметике в проекте уже обжигались трижды (см. LISTEN_TIMEOUT в
     // hls_dual_echo_krnl и docs/kernel_scheme_handoff.md): длительность надо
     // считать в проходах x фактический II, а не в тактах по частоте.
     ...
     case WAIT_STATUS:
          ...
          else if (retryTimer >= retryDelay)
          {
               // СТЕК ПРОМОЛЧАЛ -- ПОВТОРЯЕМ ЗАПРОС.
               //
               // Без этой ветки автомат оставался в WAIT_STATUS НАВСЕГДА: условие
               // выхода было только «пришёл open_status», а если он не приходит,
               // ждать нечего и незачем. Дыра достижимая, а не теоретическая: на
               // старте серверная половина ещё не открыла listen-порт, и первый
               // open_connection штатно остаётся без ответа.
               //
               // Симптом снаружи: connAttempts=1 и полная тишина -- НЕОТЛИЧИМО от
               // «ядро не стартовало», из-за чего этот дефект и прятался за
               // барьером ap_sync_done. Измерено tb_core_ap_done: при молчащем
               // стеке за 3 млн тактов была ОДНА запись open_connection вместо
               // повторов, хотя регион работал (ap_done стадии тикал 1.28 млн раз).
               //
               // Ровно такая же ветка уже есть в epd_server_listen
               // (waitTimer >= LISTEN_TIMEOUT) -- там дыру закрыли раньше, а здесь
               // пропустили.
               retryTimer = 0;
               st = IDLE;
          }
