...
puts "   epd_server_echo. II=2 измерение НЕ ломает (время штампует"
puts "   обёртка, шкала одна), но пропускная способность просядет."
puts ""
puts "3. СТРОБЫ ap_vld — на них держится вся защёлка таймстемпов."
puts "   Ждём 'to \'ap_vld\'' у sentCount, echoRxCount, echoCount,"
puts "   recvCount: обёртка штампует cycle_counter по этим стробам."
puts "   Если у какого-то окажется 'ap_none' без vld — эта точка"
puts "   перестанет защёлкиваться, и её таймстемп будет нулём."
puts ""
puts "4. ЧТО ДОЛЖНО БЫЛО ИСЧЕЗНУТЬ из ядра (было до 12.08):"
puts "   * cycleCount     — 0 упоминаний, был FIFO-каналом cycleCount_c_U;"
puts "   * epd_latch      — стадии больше нет;"
puts "   * entry_proc     — служебная стадия под тот канал, тоже ушла;"
puts "   * ts*Fifo        — четыре FIFO; остаться должен ТОЛЬКО sessionFifo."
puts "   Ориентир: 'detected/extracted 10 process function(s)' в epd_core"
puts "   (было 12) и одна строка 'Implementing FIFO'."
puts ""
puts "5. ЗАПАС ПО ТАЙМИНГУ. Estimated Fmax против 165 МГц (6.061 нс)."
puts "   Это лишь оценка HLS; настоящий WNS даёт только impl. Ориентир:"
puts "   на прогоне 12.08 при Fmax 231 МГц вышло WNS +0.067 — вчетверо"
puts "   больше, чем у dual_echo, то есть probe оказался легче."
puts ""
