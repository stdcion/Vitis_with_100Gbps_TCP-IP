...
int main()
{
    ...
    // ─────────────────────────────────────────────────────────────────
    std::cout << "\n[10a] Маркер для фильтра врезок доехал через эхо" << std::endl;
    // ЗАЧЕМ. Фильтр на axis_net_* отличает наш кадр от чужого (ARP, чистый ACK,
    // SYN с опциями) по константе в payload[4..9] — см. net_frame_filter.v.
    // Здесь проверяется единственное, чего не может проверить модель фильтра:
    // что РЕАЛЬНОЕ ядро эту константу кладёт и что ЭХО возвращает её нетронутой.
    //
    // Байты payload лежат в слове LSB-first, поэтому payload[4..9] — это биты
    // (79,32). ВНИМАНИЕ: в фильтре те же байты проверяются как [511:464], и это
    // не противоречие — там слово КАДРА, где payload сдвинут заголовками на 54
    // байта (54*8 = 432 бита разницы). Перепутать эти два числа — значит
    // получить фильтр, который не пропускает ничего, с симптомом «minWords
    // завышен».
    {
        const unsigned long long EPD_MARKER = 0x5A3C96E1B7D2ULL;
        check(s0_delivering.words.size() >= 1, "есть хотя бы одно слово ответа");
        ap_uint<512> w0 = s0_delivering.words[0];
        ap_uint<48>  got = w0(79, 32);
        checkEq((long)(unsigned long long)got, (long)EPD_MARKER,
                "маркер в payload[4..9] после круга через эхо");
        // payload[0..3] — номер отправки; он обязан остаться отдельно от
        // маркера, иначе фильтру нечего было бы сравнивать (sent меняется).
        ap_uint<32> seq = w0(31, 0);
        std::cout << "        payload[0..3]=" << seq
                  << " (sent), payload[4..9]=0x" << std::hex
                  << (unsigned long long)got << std::dec << std::endl;
    }
