...
### Время измеряет обёртка, а не ядро (2026-08-12, третья попытка)

Первый csynth probe-ядра нашёл дефект, который csim увидеть не мог. Скаляр
`cycleCount` (общий счётчик проводом внутрь) HLS раздал двум стадиям
**несимметрично** — видно в `syn/verilog`:

```
epd_server_echo.v:100    input [31:0] cycleCount;        <- провод
epd_client_traffic.v:103  input cycleCount_c_empty_n;    <- FIFO глубины 3
epd_client_traffic.v:104  output cycleCount_c_read;
```

`entry_proc` пишет в канал `cycleCount_c_U`, клиент читает из него, эхо читает
провод. То есть `t1'`/`t2` брались из задержанного каналом значения, а `t2'`/`t1`
— с провода: **снова две шкалы**, только этажом выше. Плюс пустой канал
блокировал стадию клиента (`ap_block_state1_pp0_stage0_iter0` включал
`cycleCount_c_empty_n == 0`) — то есть к перекошенным `NET_*` добавлялся риск
вечного ожидания после исчерпания FIFO.

**Вывод, который стоит запомнить:** скаляр, меняющийся каждый такт и читаемый
ДВУМЯ стадиями, передавать в HLS нельзя — размножение остаётся на усмотрение
инструмента. `iperf` не опровержение: там `timeInCycles` читает одна стадия.

**Как сделано теперь.** Ядро время не измеряет вообще: отдаёт наружу счётчики
событий, а таймстемпы по их `ap_vld` ставит обёртка.
...
`ap_vld` — готовый строб «значение изменилось в этом такте», HLS выдаёт его сам
для выходных скаляров (проверено в `epd_core.v`). Детектор изменения не нужен.
Тот самый `ap_vld`, который для чтения телеметрии бесполезен (из-за `*_preg`
значение и так держится), здесь оказался ровно тем, что требовалось.

Из ядра убраны: `epd_latch`, четыре `tsFifo*`, скаляр `cycleCount`. Добавлен
`echoRxCount` — счётчик события `t2'` (эхо приняло запрос); раньше в этой точке
наружу не менялось ничего, поэтому обёртка не могла её увидеть. Он же закрыл
пробел в диагностике: `echoes=0` больше не путается с «запрос до эха не дошёл» —
`epd_status` теперь различает эти случаи.

Шкала стала одна ФИЗИЧЕСКИ: один регистр, четыре читателя, один домен. Обёртка
видит событие на такт позже, но одинаково для всех четырёх точек, поэтому
разности точны, а абсолютные значения не используются.

`sampleReady` тоже переехал в обёртку и упростился: вместо ожидания четырёх FIFO
— флаг «`t2` пришёл после последнего `triggerGo`».

Тестбенч теперь моделирует обёртку (`wrapper_tick()`), а не читает таймстемпы из
ядра — то есть совпадает с железом по конструкции. 40/40 зелёные, числа те же
(RTT 81/411/90).

### Чего НЕ проверено

* ~~**`sampleReady` и `*_preg`**~~ — **закрыто.** Первый csynth подтвердил
  `sampleReady_preg` (и `tsRequest_preg`, `tsEchoIn_preg`, `tsEchoOut_preg`,
  `tsReply_preg`) — значения держались корректно, латч не требовался. Вопрос
  снят окончательно: `sampleReady` и все четыре таймстемпа переехали в обёртку,
  в ядре их больше нет.
