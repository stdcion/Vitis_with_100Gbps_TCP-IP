/**********
Host для hls_pingpong_probe_krnl (вариант с измерением задержки).

Отличается от host для hls_pingpong_krnl только именем ядра, которое
ищется в битстриме. Само измерение — в ядре: оно вписывает число
тактов обработки в первые 4 байта эха.

Настраивает сетевое ядро (локальный IP/MAC) и запускает echo-ядро,
которое дальше работает само. Клиент читает счётчик:
    ./pingpong_client <fpga_ip> 5001 64 100000 5000 --probe
...
int main(int argc, char **argv) {
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, user_kernel = cl::Kernel(program, "hls_pingpong_probe_krnl", &err));
            valid_device++;
            break;
        }
    }
